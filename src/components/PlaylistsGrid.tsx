...
export default function PlaylistsGrid({ playlists }: { playlists: Playlist[] }) {
  return (
    <div className="grid min-w-0 grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
      {playlists.map((playlist) => (
        ...
        >
          <img
            ...
            alt={playlist.name}
            className="w-36 h-36 object-cover rounded-lg mb-2 shadow-md"
          />
          <div className="text-left w-40 min-w-0">
            <div className="font-semibold text-lg truncate" title={playlist.name}>
              {playlist.name}
            </div>
